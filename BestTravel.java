...
public class BestTravel {
    ...
    public static Integer chooseBestSum(int t, int k, List<Integer> ls) {
        if(ls.size() < k || ls.isEmpty()) return null;
        int tmp = ls.stream().sorted().limit(k).reduce((x, y) -> x + y).get();
        if(tmp == t){
            return tmp;
        }else{
            ...
            Iterator<Integer> iterator = sums.descendingIterator();
    ...
    public static void calculateSums(List<Integer> ls, int k, int s, int ind){
        if(ind == k){
            sums.add(IntStream.of(variations).sum());
            return;
        }
        for(int i = s; i < ls.size(); i++){
            variations[ind] = ls.get(i);
            calculateSums(ls, k, i+1, ind+1);
        }
    }
}